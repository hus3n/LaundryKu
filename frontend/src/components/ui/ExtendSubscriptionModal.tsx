...
export default function ExtendSubscriptionModal({
  ...
  return (
    <>
      ...
      >
        <div className="glass-card-dark p-6 sm:p-7 rounded-3xl border dark:border-[#1DA9D0]/15 border-slate-200 max-w-lg w-full shadow-2xl relative space-y-6 pointer-events-auto">
          ...
          <div>
            <h3 className="text-lg font-bold dark:text-[#F5EACA] text-slate-900 flex items-center gap-2">
              Perpanjang Masa Aktif Toko
            </h3>
            <p className="text-xs dark:text-[#F5EACA]/60 text-slate-500 mt-1">
              Tambahkan durasi langganan untuk <span className="text-emerald-600 dark:text-emerald-300 font-semibold">{admin.storeName}</span>
            </p>
          </div>
        </div>

        {/* Info Card current subscription */}
        <div className="p-3.5 rounded-2xl dark:bg-[#012040] bg-slate-50 border dark:border-[#1DA9D0]/15 border-slate-200 grid grid-cols-2 gap-3 text-xs">
          <div>
            <span className="text-[11px] dark:text-[#F5EACA]/60 text-slate-500 block font-medium">Pemilik Toko</span>
            <span className="dark:text-[#F5EACA] text-slate-800 font-semibold">{admin.user?.name || '-'}</span>
          </div>
          <div>
            <span className="text-[11px] dark:text-[#F5EACA]/60 text-slate-500 block font-medium">Berakhir Saat Ini</span>
            <span className={`font-semibold ${new Date(admin.subscriptionEnd) < new Date() ? 'text-rose-500 dark:text-rose-400' : 'dark:text-[#F5EACA] text-slate-800'}`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-xs font-semibold dark:text-[#F5EACA]/80 text-slate-700 mb-2.5">
                ...
                <button
                  ...
                  className={`p-3 rounded-2xl border text-left transition-all relative overflow-hidden flex flex-col justify-between ${
                    !isCustom && months === opt.value
                      ? 'bg-emerald-500/10 border-emerald-500/50 dark:text-[#F5EACA] text-slate-900 shadow-lg shadow-emerald-500/10'
                      : 'dark:bg-[#012040] bg-white dark:border-[#1DA9D0]/15 border-slate-200 dark:text-[#F5EACA]/60 text-slate-600 hover:border-slate-300 dark:hover:border-[#1DA9D0]/40'
                  }`}
                >
              ...
              {isCustom && (
                <div className="mt-2">
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl dark:bg-[#012040] bg-white border dark:border-[#1DA9D0]/25 border-slate-300 text-xs dark:text-[#F5EACA] text-slate-900 dark:placeholder-[#1DA9D0]/40 placeholder-slate-400 focus:outline-none focus:border-emerald-500"
                  />
                </div>
              )}
            </div>
