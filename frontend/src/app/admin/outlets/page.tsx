    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Kelola Outlet</h1>
            <p className="text-xs text-foreground/60 mt-1">Daftar cabang/outlet dari toko Anda</p>
          </div>
          <button
        ...
        <div className="glass-card-dark rounded-2xl border border-[#1DA9D0]/15 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-surface border-b border-[#1DA9D0]/15 text-foreground/60">
                <tr>
              ...
              <tbody className="divide-y divide-[#1DA9D0]/10">
                {error ? (
                  ...
                  <tr>
                    <td colSpan={4} className="py-8 text-center text-foreground/60">Memuat data...</td>
                  </tr>
                ) : outlets.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="py-12 text-center text-foreground/60 space-y-3">
                  ...
                  outlets.map((outlet) => (
                    <tr key={outlet.id} className="hover:bg-muted/50 transition-colors">
                      <td className="py-3 px-4 text-foreground font-medium">{outlet.name}</td>
                      <td className="py-3 px-4 text-foreground/80">{outlet.address || '—'}</td>
                      <td className="py-3 px-4 text-foreground/80">{outlet.phone || '—'}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/85 backdrop-blur-sm">
            <div className="bg-surface p-6 rounded-3xl border border-[#1DA9D0]/25 max-w-md w-full space-y-4 shadow-2xl">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-foreground/80 mb-1">Nama Outlet *</label>
                  <input
                    type="text"
                    required
                    minLength={2}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3.5 py-2 rounded-xl bg-muted border border-[#1DA9D0]/25 text-xs text-foreground placeholder-[#1DA9D0]/50 focus:outline-none focus:border-[#1DA9D0] resize-none"
                  />
                </div>

                <div>
                  <label className="block text-xs font-semibold text-foreground/80 mb-1">Nomor Telepon (Opsional)</label>
                  <input
                    type="text"
                    value={phone}
