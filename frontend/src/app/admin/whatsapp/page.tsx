  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Integrasi WhatsApp Toko</h1>
          <p className="text-xs text-foreground/60 mt-1">Pairing perangkat WhatsApp gateway, atur template pesan, dan pengiriman notifikasi</p>
        </div>
          ...
          <div className="p-4 rounded-xl border border-[#EA8803]/30 bg-[#EA8803]/10 mb-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-foreground/80 mt-1">{subscriptionError}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 glass-card-dark p-6 rounded-3xl border border-[#1DA9D0]/15 space-y-6 text-center">
            <div className="flex justify-between items-center text-xs">
              <span className="text-foreground/60">Status Koneksi WA:</span>
              <span
            ...
            {pendingQueueCount > 0 && (
              <div className="p-3.5 rounded-2xl bg-[#EA8803]/10 border border-[#EA8803]/30 text-[#EA8803] text-xs space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-foreground bg-[#EA8803]/20 px-2.5 py-0.5 rounded-full border border-[#EA8803]/40">
                ...
                <div className="flex gap-2 pt-1">
                  {status === 'CONNECTED' && (
                    <button
                      ...
                      className="flex-1 py-1 px-2 rounded-lg bg-[#EA8803]/20 hover:bg-[#EA8803]/30 border border-[#EA8803]/40 text-foreground text-[10px] font-semibold flex items-center justify-center gap-1 transition-all"
                    >
            ...
            {/* QR Container */}
            <div className="p-4 rounded-2xl bg-surface-deep border border-[#1DA9D0]/15 flex flex-col items-center justify-center min-h-[220px]">
              {status === 'CONNECTED' ? (
                <div className="space-y-3 py-4">
                  <div className="w-16 h-16 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center mx-auto border border-emerald-500/30">
                    <CheckCircle2 className="w-10 h-10" />
                  </div>
                  <h4 className="font-bold text-foreground text-sm">WA Toko Aktif</h4>
                  <p className="text-xs text-foreground/60">{phoneConnected || '0812-3456-7890'}</p>
                </div>
              ) : qrCode ? (
                ...
                <div className="space-y-3 py-6">
                  ...
                  <p className="text-xs text-foreground/60">Klik &quot;Hubungkan WA&quot; untuk generate QR Code</p>
                </div>
              )}
            </div>
          ...
          <div className="md:col-span-2 glass-card-dark p-6 rounded-3xl border border-[#1DA9D0]/15 space-y-6">
            ...
            <div className="space-y-3">
              <textarea
                ...
                className="w-full p-4 rounded-xl bg-surface border border-[#1DA9D0]/25 text-xs text-foreground placeholder-[#1DA9D0]/50 focus:outline-none focus:border-[#1DA9D0] font-mono leading-relaxed"
              />

              {/* Dynamic Variables Guide */}
              <div className="p-3.5 rounded-xl bg-surface/60 border border-[#1DA9D0]/15 text-[11px] text-foreground/60 space-y-1">
                <p className="font-semibold text-foreground/80">Variabel Dinamis yang Tersedia:</p>
                <div className="flex flex-wrap gap-2 text-[10px]">
                  <span className="px-2 py-0.5 rounded bg-muted text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;nama_pelanggan&#125;&#125;</span>
                  <span className="px-2 py-0.5 rounded bg-muted text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;no_nota&#125;&#125;</span>
                  <span className="px-2 py-0.5 rounded bg-muted text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;detail_item&#125;&#125;</span>
                  <span className="px-2 py-0.5 rounded bg-muted text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;total_harga&#125;&#125;</span>
                  <span className="px-2 py-0.5 rounded bg-muted text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;status_bayar&#125;&#125;</span>
                  <span className="px-2 py-0.5 rounded bg-muted text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;estimasi_selesai&#125;&#125;</span>
                  <span className="px-2 py-0.5 rounded bg-muted text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;nama_toko&#125;&#125;</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="glass-card-dark p-6 rounded-3xl border border-[#1DA9D0]/15 space-y-4">
          ...
          <form onSubmit={handleSendCustom} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-foreground/80 mb-1">Nama Penerima</label>
                <input
                  type="text"
                  required
                  value={customName}
              ...
              <div>
                <label className="block text-xs font-semibold text-foreground/80 mb-1">Nomor WA Penerima</label>
                <input
                  type="text"
                  required
                  value={customPhone}
            ...
            <div>
              <label className="block text-xs font-semibold text-foreground/80 mb-1">Isi Pesan Custom</label>
              <textarea
                rows={3}
                required
                value={customMsg}
                ...
                className="w-full p-3.5 rounded-xl bg-surface border border-[#1DA9D0]/25 text-xs text-foreground focus:outline-none focus:border-[#1DA9D0]"
              />
            </div>
