  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        <div>
          <h1 className="text-2xl font-bold text-[#F5EACA]">Integrasi WhatsApp Toko</h1>
          <p className="text-xs text-[#F5EACA]/60 mt-1">Pairing perangkat WhatsApp gateway, atur template pesan, dan pengiriman notifikasi</p>
        </div>

        {subscriptionError && (
          <div className="p-4 rounded-xl border border-[#EA8803]/30 bg-[#EA8803]/10 mb-6">
            <div className="flex items-start gap-3">
              <AlertCircle className="w-5 h-5 text-[#EA8803] mt-0.5 flex-shrink-0" />
              <div>
                <h3 className="font-medium text-[#EA8803]">
                  {subscriptionCode === 'SUBSCRIPTION_EXPIRED'
                    ? 'Masa Aktif Akun Telah Berakhir'
                    : 'Akun Tidak Aktif'}
                </h3>
                <p className="text-sm text-[#F5EACA]/80 mt-1">{subscriptionError}</p>
                {subscriptionCode !== 'ACCOUNT_INACTIVE' && (
                  <p className="text-sm text-[#EA8803] mt-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 glass-card-dark p-6 rounded-3xl border border-[#1DA9D0]/15 space-y-6 text-center">
            <div className="flex justify-between items-center text-xs">
              <span className="text-[#F5EACA]/60">Status Koneksi WA:</span>
              <span
                className={`px-3 py-1 rounded-full font-bold flex items-center gap-1.5 ${
                  status === 'CONNECTED'
                    ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30'
                    : status === 'CONNECTING'
                    ? 'bg-[#EA8803]/20 text-[#EA8803] border border-[#EA8803]/30'
            ...
            {pendingQueueCount > 0 && (
              <div className="p-3.5 rounded-2xl bg-[#EA8803]/10 border border-[#EA8803]/30 text-[#EA8803] text-xs space-y-2">
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 font-medium">
                    <Clock className="w-3.5 h-3.5 text-[#EA8803]" /> Antrean Tertahan:
                  </span>
                  <span className="font-bold text-[#F5EACA] bg-[#EA8803]/20 px-2.5 py-0.5 rounded-full border border-[#EA8803]/40">
                    {pendingQueueCount} Pesan
                  </span>
                </div>
                <p className="text-[10px] text-[#EA8803]/80 text-left">
                  {status === 'CONNECTED'
                ...
                <div className="flex gap-2 pt-1">
                  {status === 'CONNECTED' && (
                    <button
                      ...
                      className="flex-1 py-1 px-2 rounded-lg bg-[#EA8803]/20 hover:bg-[#EA8803]/30 border border-[#EA8803]/40 text-[#F5EACA] text-[10px] font-semibold flex items-center justify-center gap-1 transition-all"
                    >
            ...
            {/* QR Container */}
            <div className="p-4 rounded-2xl bg-[#011B2E] border border-[#1DA9D0]/15 flex flex-col items-center justify-center min-h-[220px]">
              {status === 'CONNECTED' ? (
                <div className="space-y-3 py-4">
                  <div className="w-16 h-16 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center mx-auto border border-emerald-500/30">
                    <CheckCircle2 className="w-10 h-10" />
                  </div>
                  <h4 className="font-bold text-[#F5EACA] text-sm">WA Toko Aktif</h4>
                  <p className="text-xs text-[#F5EACA]/60">{phoneConnected || '0812-3456-7890'}</p>
                </div>
              ) : qrCode ? (
                <div className="space-y-3">
                  ...
                  <p className="text-[11px] text-[#EA8803]">Scan QR Code dengan WhatsApp HP Toko</p>
                  <button
                ...
                <div className="space-y-3 py-6">
                  <QrCode className="w-12 h-12 text-[#1DA9D0]/40 mx-auto" />
                  <p className="text-xs text-[#F5EACA]/60">Klik &quot;Hubungkan WA&quot; untuk generate QR Code</p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              {status === 'CONNECTED' ? (
                <div className="space-y-2">
                  ...
                  <p className="text-[10px] text-[#F5EACA]/60">
          ...
          {/* Template Editor */}
          <div className="md:col-span-2 glass-card-dark p-6 rounded-3xl border border-[#1DA9D0]/15 space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-[#F5EACA] flex items-center gap-2">
                <Code className="w-4 h-4 text-[#43D5CC]" /> Editor Template Pesan WA
              </h3>
              <button
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-all border ${
                    selectedTemplateId === tmpl._id
                      ...
                      : 'bg-[#012040] text-[#F5EACA]/60 border-[#1DA9D0]/15 hover:text-[#F5EACA] hover:bg-[#013D66]'
                  }`}
                >
                  {tmpl.name}
                </button>
            ...
            <div className="space-y-3">
              <textarea
                ...
                className="w-full p-4 rounded-xl bg-[#012040] border border-[#1DA9D0]/25 text-xs text-[#F5EACA] placeholder-[#1DA9D0]/50 focus:outline-none focus:border-[#1DA9D0] font-mono leading-relaxed"
              />

              {/* Dynamic Variables Guide */}
              <div className="p-3.5 rounded-xl bg-[#012040]/60 border border-[#1DA9D0]/15 text-[11px] text-[#F5EACA]/60 space-y-1">
                <p className="font-semibold text-[#F5EACA]/80">Variabel Dinamis yang Tersedia:</p>
                <div className="flex flex-wrap gap-2 text-[10px]">
                  <span className="px-2 py-0.5 rounded bg-[#013D66] text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;nama_pelanggan&#125;&#125;</span>
                  <span className="px-2 py-0.5 rounded bg-[#013D66] text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;no_nota&#125;&#125;</span>
                  <span className="px-2 py-0.5 rounded bg-[#013D66] text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;detail_item&#125;&#125;</span>
                  <span className="px-2 py-0.5 rounded bg-[#013D66] text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;total_harga&#125;&#125;</span>
                  <span className="px-2 py-0.5 rounded bg-[#013D66] text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;status_bayar&#125;&#125;</span>
                  <span className="px-2 py-0.5 rounded bg-[#013D66] text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;estimasi_selesai&#125;&#125;</span>
                  <span className="px-2 py-0.5 rounded bg-[#013D66] text-[#43D5CC] border border-[#1DA9D0]/25 font-mono">&#123;&#123;nama_toko&#125;&#125;</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="glass-card-dark p-6 rounded-3xl border border-[#1DA9D0]/15 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-bold text-[#F5EACA] flex items-center gap-2">
              <MessageSquare className="w-4 h-4 text-[#43D5CC]" /> Kirim Pesan Custom via WA Toko
            </h3>
            <span className="text-[11px] text-[#EA8803] flex items-center gap-1">
          ...
          <form onSubmit={handleSendCustom} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-[#F5EACA]/80 mb-1">Nama Penerima</label>
                <input
                  type="text"
                  required
                  value={customName}
              ...
              <div>
                <label className="block text-xs font-semibold text-[#F5EACA]/80 mb-1">Nomor WA Penerima</label>
                <input
                  type="text"
                  required
                  value={customPhone}
            ...
            <div>
              <label className="block text-xs font-semibold text-[#F5EACA]/80 mb-1">Isi Pesan Custom</label>
              <textarea
                rows={3}
                required
                value={customMsg}
                ...
                className="w-full p-3.5 rounded-xl bg-[#012040] border border-[#1DA9D0]/25 text-xs text-[#F5EACA] focus:outline-none focus:border-[#1DA9D0]"
              />
            </div>
