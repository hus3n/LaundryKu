  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Log Aktivitas Sistem</h1>
          <p className="text-xs text-foreground/60 mt-1">Audit trail seluruh aktivitas pengguna di sistem toko laundry Anda</p>
        </div>
        ...
        <div className="glass-card-dark rounded-2xl border border-[#1DA9D0]/15 overflow-hidden">
          {error ? (
            ...
            <div className="text-center py-12 text-xs text-foreground/60">Memuat log aktivitas...</div>
          ) : logs.length === 0 ? (
                ...
                <tbody className="divide-y divide-[#1DA9D0]/10">
                  {logs.map((log) => (
                    <tr key={log.id} className="hover:bg-muted/50 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-foreground/80">{log.entity}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
