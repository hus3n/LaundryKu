    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold dark:text-[#F5EACA] text-slate-900">Kelola Paket Layanan</h1>
            <p className="text-xs dark:text-[#F5EACA]/60 text-slate-500 mt-1">Pengaturan jenis paket cucian, harga, dan satuan unit toko</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {error ? (
            ...
            <div className="col-span-3 text-center py-12 text-xs dark:text-[#F5EACA]/60 text-slate-500">Memuat data paket...</div>
          ) : packages.length === 0 ? (
            <div className="col-span-3 glass-card-dark p-12 rounded-2xl border dark:border-[#1DA9D0]/15 border-slate-200 text-center text-xs dark:text-[#F5EACA]/60 text-slate-500 space-y-3 shadow-sm">
              <PackageIcon className="w-10 h-10 mx-auto dark:text-[#1DA9D0]/40 text-slate-400" />
              <p>Belum ada paket layanan ditambahkan.</p>
            </div>
          ) : (
            packages.map((pkg) => (
              <div key={pkg.id} className="glass-card-dark p-6 rounded-2xl border dark:border-[#1DA9D0]/15 border-slate-200 space-y-4 relative shadow-sm">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-bold dark:text-[#F5EACA] text-slate-900 text-base">{pkg.name}</h3>
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="p-2 rounded-lg dark:text-[#F5EACA]/60 text-slate-500 dark:hover:text-[#43D5CC] hover:text-teal-600 dark:hover:bg-[#013D66] hover:bg-slate-100 transition-colors"
                    >
                      <Edit className="w-4 h-4" />
                    </button>
                    <button
                      onClick={() => handleDelete(pkg.id)}
                      className="p-2 rounded-lg dark:text-[#F5EACA]/60 text-slate-500 hover:text-rose-500 dark:hover:bg-[#013D66] hover:bg-slate-100 transition-colors"
                    >
                ...
                <div className="pt-2 border-t dark:border-[#1DA9D0]/15 border-slate-200 flex justify-between items-center text-xs">
                  ...
                  <span className="text-base font-bold dark:text-[#43D5CC] text-teal-600">
                    Rp {Number(pkg.price).toLocaleString('id-ID')}
                  </span>
                </div>

                <div className="text-[11px] dark:text-[#F5EACA]/60 text-slate-500 flex items-center gap-1.5">
                  <Clock className="w-3.5 h-3.5 dark:text-[#EA8803] text-amber-500" />
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 dark:bg-[#010E1C]/85 bg-slate-900/40 backdrop-blur-sm">
            <div className="dark:bg-[#012040] bg-white p-6 rounded-3xl border dark:border-[#1DA9D0]/25 border-slate-200 max-w-md w-full space-y-4 shadow-2xl">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold dark:text-[#F5EACA]/80 text-slate-700 mb-1">Nama Paket</label>
                  <input
                    type="text"
                    required
                    value={name}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold dark:text-[#F5EACA]/80 text-slate-700 mb-1">Satuan Unit</label>
                    <input
                      type="text"
                      required
                      value={unit}
                      ...
                      className="w-full px-3.5 py-2 rounded-xl dark:bg-[#013D66] bg-slate-50 border dark:border-[#1DA9D0]/25 border-slate-300 text-xs dark:text-[#F5EACA] text-slate-900 focus:outline-none focus:border-[#1DA9D0]"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-semibold dark:text-[#F5EACA]/80 text-slate-700 mb-1">Harga (Rp)</label>
                    <input
                      type="number"
                      required
                      value={price}
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <select
                      ...
                      className="px-3.5 py-2 rounded-xl dark:bg-[#013D66] bg-slate-50 border dark:border-[#1DA9D0]/25 border-slate-300 text-xs dark:text-[#F5EACA] text-slate-900 focus:outline-none focus:border-[#1DA9D0] w-24 shrink-0"
                    >
