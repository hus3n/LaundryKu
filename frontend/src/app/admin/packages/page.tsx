    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Kelola Paket Layanan</h1>
            <p className="text-xs text-foreground/60 mt-1">Pengaturan jenis paket cucian, harga, dan satuan unit toko</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {error ? (
            ...
            <div className="col-span-3 text-center py-12 text-xs text-foreground/60">Memuat data paket...</div>
          ) : packages.length === 0 ? (
            ...
            packages.map((pkg) => (
              <div key={pkg.id} className="glass-card-dark p-6 rounded-2xl border border-[#1DA9D0]/15 space-y-4 relative">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="p-2 rounded-lg text-foreground/60 hover:text-[#43D5CC] hover:bg-muted transition-colors"
                    >
                      <Edit className="w-4 h-4" />
                    </button>
                    <button
                      onClick={() => handleDelete(pkg.id)}
                      className="p-2 rounded-lg text-foreground/60 hover:text-rose-400 hover:bg-muted transition-colors"
                    >
                ...
                <div className="pt-2 border-t border-[#1DA9D0]/15 flex justify-between items-center text-xs">
                  <span className="text-foreground/60">Harga Layanan:</span>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/85 backdrop-blur-sm">
            <div className="bg-surface p-6 rounded-3xl border border-[#1DA9D0]/25 max-w-md w-full space-y-4 shadow-2xl">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-foreground/80 mb-1">Nama Paket</label>
                  <input
                    type="text"
                    required
                    value={name}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-foreground/80 mb-1">Satuan Unit</label>
                    <input
                      type="text"
                      required
                      value={unit}
                      ...
                      className="w-full px-3.5 py-2 rounded-xl bg-muted border border-[#1DA9D0]/25 text-xs text-foreground focus:outline-none focus:border-[#1DA9D0]"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-semibold text-foreground/80 mb-1">Harga (Rp)</label>
                    <input
                      type="number"
                      required
                      value={price}
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <select
                      ...
                      className="px-3.5 py-2 rounded-xl bg-muted border border-[#1DA9D0]/25 text-xs text-foreground focus:outline-none focus:border-[#1DA9D0] w-24 shrink-0"
                    >
