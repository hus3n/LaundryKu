    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Kelola Data Pelanggan</h1>
            <p className="text-xs text-foreground/60 mt-1">Database kontak pelanggan toko dan nomor WhatsApp terdaftar</p>
          </div>
          <div className="flex gap-3">
            <button
              onClick={handleDownloadCustomers}
              className="px-4 py-2.5 rounded-xl bg-muted hover:bg-muted-hover text-foreground font-semibold text-xs border border-[#1DA9D0]/25 shadow-lg transition-all inline-flex items-center gap-2"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {error ? (
            ...
            <div className="col-span-3 text-center py-12 text-xs text-foreground/60">Memuat data pelanggan...</div>
          ) : customers.length === 0 ? (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-bold text-foreground text-sm">{cust.name}</h4>
                        <a
                  ...
                  {cust.address && (
                    <div className="text-xs text-foreground/60 pt-2 border-t border-[#1DA9D0]/15 flex items-start gap-1.5">
        ...
        <AnimatePresence>
          {modalOpen && (
            <>
              ...
              >
                <div className="bg-surface p-6 rounded-3xl border border-[#1DA9D0]/25 max-w-md w-full space-y-4 pointer-events-auto shadow-2xl">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                      <label className="block text-xs font-semibold text-foreground/80 mb-1">Nama Pelanggan</label>
                      <input
                        type="text"
                        required
                        value={name}
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-foreground/80 mb-1">Nomor WhatsApp</label>
                      <input
                        type="text"
                        required
                        value={phone}
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-foreground/80 mb-1">Alamat (Opsional)</label>
                      <textarea
                        rows={2}
                        value={address}
