    ...
    <DashboardLayout>
      <div className="space-y-3.5 sm:space-y-4 md:space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <div>
            <h1 className="text-lg sm:text-xl md:text-2xl font-bold dark:text-[#F5EACA] text-slate-900">Kelola Data Pelanggan</h1>
            <p className="text-[11px] sm:text-xs dark:text-[#F5EACA]/60 text-slate-500 mt-0.5 sm:mt-1">Database kontak pelanggan toko dan nomor WhatsApp terdaftar</p>
          </div>
          <div className="flex gap-2 sm:gap-3">
            <button
              onClick={handleOpenCreate}
              className="px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-xl bg-gradient-to-r from-[#1DA9D0] to-[#43D5CC] hover:opacity-95 text-[#010E1C] font-bold text-xs shadow-md shadow-[#1DA9D0]/20 transition-all inline-flex items-center gap-1.5 sm:gap-2"
            >
        ...
        {/* Search */}
        <div className="glass-card-dark p-3 sm:p-4 rounded-xl sm:rounded-2xl border dark:border-[#1DA9D0]/15 border-slate-200 flex justify-between items-center shadow-sm">
          <form onSubmit={handleSearchSubmit} className="relative w-full max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-3.5 py-2 rounded-xl dark:bg-[#012040] bg-white border dark:border-[#1DA9D0]/25 border-slate-300 text-xs dark:text-[#F5EACA] text-slate-900 dark:placeholder-[#1DA9D0]/50 placeholder-slate-400 focus:outline-none focus:border-[#1DA9D0] shadow-sm"
            />
          </form>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-5">
          {error ? (
            <div className="col-span-full text-center py-8 text-xs text-rose-500">⚠️ {error}</div>
          ) : loading ? (
            <div className="col-span-full text-center py-8 text-xs dark:text-[#F5EACA]/60 text-slate-500">Memuat data pelanggan...</div>
          ) : customers.length === 0 ? (
            <div className="col-span-full glass-card-dark p-8 sm:p-12 rounded-xl sm:rounded-2xl border dark:border-[#1DA9D0]/15 border-slate-200 text-center text-xs dark:text-[#F5EACA]/60 text-slate-500 space-y-2 sm:space-y-3 shadow-sm">
              <Users className="w-8 h-8 sm:w-10 sm:h-10 mx-auto dark:text-[#1DA9D0]/40 text-slate-400" />
            ...
            <AnimatePresence>
              {customers.map((cust) => (
                <motion.div 
                  ...
                  layout
                  className="glass-card-dark p-3.5 sm:p-4 md:p-5 rounded-xl sm:rounded-2xl border dark:border-[#1DA9D0]/15 border-slate-200 space-y-2.5 sm:space-y-3 shadow-sm"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5 sm:gap-3">
                      <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl dark:bg-[#1DA9D0]/20 bg-teal-50 border dark:border-[#1DA9D0]/30 border-teal-200 flex items-center justify-center font-bold dark:text-[#43D5CC] text-teal-700 text-xs sm:text-sm">
                  ...
                  {cust.address && (
                    <div className="text-[11px] sm:text-xs dark:text-[#F5EACA]/60 text-slate-500 pt-2 border-t dark:border-[#1DA9D0]/15 border-slate-200 flex items-start gap-1.5">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
                    ...
                    <div className="pt-2 sm:pt-3 flex justify-end gap-2 sm:gap-3">
                      ...
                      <motion.button
                        ...
                        className="px-4 sm:px-5 py-1.5 sm:py-2 rounded-xl bg-gradient-to-r from-[#1DA9D0] to-[#43D5CC] hover:opacity-95 text-[#010E1C] font-bold text-xs disabled:opacity-50 transition-all shadow-sm"
                      >
