    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Kelola Kategori Cucian</h1>
            <p className="text-xs text-foreground/60 mt-1">Kategori jenis barang (Kiloan, Satuan, Bed Cover, Karpet, Sepatu, dll)</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {error ? (
            ...
            <div className="col-span-4 text-center py-12 text-xs text-foreground/60">Memuat data kategori...</div>
          ) : categories.length === 0 ? (
            <div className="col-span-4 glass-card-dark p-12 rounded-2xl text-center text-xs text-foreground/60 space-y-3">
        ...
        {modalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/85 backdrop-blur-sm">
            <div className="bg-surface p-6 rounded-3xl border border-[#1DA9D0]/25 max-w-sm w-full space-y-4 shadow-2xl">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-foreground/80 mb-1">Nama Kategori</label>
                  <input
                    type="text"
                    required
                    value={name}
